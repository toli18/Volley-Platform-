...
@router.post("/", response_model=TrainingRead, dependencies=[Depends(require_role(UserRole.coach, UserRole.bfv_admin, UserRole.platform_admin))])
def create_training(payload: TrainingCreate, db: Session = Depends(get_db_session), user=Depends(get_current_user)):
    training = Training(
